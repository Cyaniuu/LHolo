...
- 修复 LHolo 菜单绘制过一次后调整游戏窗口大小可能触发 `0xC0000409` 闪退的问题；现在同时覆盖 D3D11 的 `ResizeBuffers` 与 D3D12 的 `ResizeBuffers1`，在交换链缩放前完整释放 D3D11On12 图形后端，并在稳定的后续 Present 中懒重建。
- ImGui Overlay 现在只处理已绑定的游戏交换链，并在图形初始化失败时统一回滚部分创建的 COM 资源，避免与其他模组的离屏交换链互相影响或在重试中泄漏资源。
